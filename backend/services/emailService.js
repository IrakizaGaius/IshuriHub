...
const sendPasswordResetEmail = async (email, token) => {
  const resetUrl = `https://ishurihub-production.up.railway.app/api/users/reset-password?token=${token}`;
  const mailOptions = {
    to: email,
...
const sendEmailVerification = async (email, token) => {
  const verificationUrl = `https://ishurihub-production.up.railway.app/api/users/verify-email?token=${token}`;
  const mailOptions = {
    to: email,
    from: process.env.EMAIL_USER,
